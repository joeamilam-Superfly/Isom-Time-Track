...
- There's a "View my schedule" button on the My Hours tab showing roughly the next 4 weeks of job-site assignments, set by a foreman or admin (employees can't set their own). If a foreman or admin changes a schedule assignment, the employee gets shown a notification the next time they open the app and must tap to confirm they saw it - if the change affects TODAY specifically, they also get a text message. Foremen and admins manage scheduling from a grid inside the Approvals tab (a "Schedule" toggle there).
